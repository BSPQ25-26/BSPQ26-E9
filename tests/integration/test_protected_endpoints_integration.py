...
def _create_and_login_user(client: httpx.Client, prefix: str) -> dict:
    ...
    try:
        ...
        pytest.skip(f"Auth service is not reachable at {AUTH_BASE_URL}: {exc}")
...
def _create_product(client: httpx.Client, owner_token: str) -> dict:
    ...
    try:
        ...
        pytest.skip(f"Inventory service is not reachable at {INVENTORY_BASE_URL}: {exc}")
...
def _request_by_method(client: httpx.Client, method: str, url: str, token: str | None = None, json: dict | None = None):
...
def test_private_product_endpoints_require_token_and_enforce_ownership(
    ...
    assert without_token.status_code == 401, without_token.text

    unauthorized_user = _request_by_method(api_client, method, url, token=outsider["token"])
    assert unauthorized_user.status_code == 403, unauthorized_user.text
